...
this.buttonNode = cdp.querySelector('.govuk-form-group button');
...
updateDate(domNode, disable, day, selected) {
  ...
  domNode.classList.remove('current-day'); // <<< Add this
  ...
  if (disable) {
    ...
    // Add current-day class if matches today
    ...
    if (
      ...
      domNode.classList.add('current-day'); // <<< Add this
    }
  }
}
...
setTextboxDate(domNode) {
  ...
  if (d < today) {
    window.location.href = "/enter-a-date-for-a-slot?enter-a-date-for-a-slot-error=true";
    return;
  }
  // Save ISO date string for robust parsing
  localStorage.setItem('chosenAppointmentDate', d.toISOString());
